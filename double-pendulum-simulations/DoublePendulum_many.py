...
global sim,follow
sim = int(math.pow(2,11))
#follow = [0,1,3,15,31,63,127,255]
follow = []

def update(win, pendulums, points, showBeams=False, showOther=True):
    global sim
...
timestep = 0.1
initalAngle = math.pi*0.25
angleStep = 1.5*math.pi/sim

win = Window(400,400,2)
allPairs = []
for i in range(sim):
    allPairs.append([Pendulum(200,200,90,10,initalAngle+angleStep*i,1),Pendulum(200+90*math.sin(initalAngle),200+90*math.cos(initalAngle),90,10,2*(initalAngle+angleStep*i),1)])
ps = []
